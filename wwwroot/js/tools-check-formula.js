(() => {
  ...
  $('#checkFormulaGenerate').addEventListener('click', async () => {
    ...
    try {
      if (window.XLSX) {
        const rows = [['類型', '代碼', '說明', '公式', '特殊月份', '特殊公式']];
        state.selfChecks.forEach(item => rows.push(['自身檢核', item.code, item.description, item.formula, '', item.note]));
        state.crossChecks.forEach(item => rows.push(['交叉檢核', item.code, item.description, item.formula, item.specialMonth, item.specialFormula]));
        const workbook = XLSX.utils.book_new();
        const worksheet = XLSX.utils.aoa_to_sheet(rows);
        worksheet['!cols'] = [{ wch: 14 }, { wch: 24 }, { wch: 36 }, { wch: 60 }, { wch: 14 }, { wch: 36 }];
        XLSX.utils.book_append_sheet(workbook, worksheet, $('#checkTemplateId').value);
        XLSX.writeFile(workbook, `${$('#checkTemplateId').value}_CheckFormula_${Date.now()}.xlsx`);
        $('#checkFormulaStatus').textContent = `Excel 已產生，共 ${rows.length - 1} 筆`;
